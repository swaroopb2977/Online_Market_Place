...
def new_conversation(request, item_pk):
    ...
    existing_conversation = Conversation.objects.filter(item=item, member=request.user).first()
...
def detail(request, pk):
    conversation = Conversation.objects.filter(member=request.user).get(pk=pk)
